...
Windows에서 **콘솔(cmd) 창 없이** 실행하려면 PyInstaller에 “GUI 모드”로 빌드해주면 돼요.

### 가장 간단한 방법 (권장)

```bash
# Windows
pyinstaller ScentSmart.py --clean --windowed  ^
  --add-data "ui;ui"
```

- `--windowed`(= `--noconsole`) : 콘솔 창 숨김
    
- 리소스 포함은 그대로 `--add-data` 사용(위는 예시)
    

> macOS/Linux라면 `--windowed`만 주면 되고, `--add-data "ui:ui"`처럼 콜론(`:`)을 써요.

### spec 파일을 쓰는 경우

`ScentSmart.spec`의 EXE 설정에서 콘솔을 꺼주세요.

```python
exe = EXE(
    ...,
    console=False,   # or windowed=True
)
```
...
```bash
pyinstaller ScentSmart.spec
```

### 개발 중에만 콘솔 없이 테스트하고 싶다면

```bash
pythonw ScentSmart.py   # (Windows 전용, 콘솔 없는 파이썬 런처)
```

### 참고

- `print()`나 `logging` 출력은 콘솔이 없어서 화면에 안 보입니다. 필요하면 파일로 리다이렉트하세요.
    
    ```python
    if getattr(sys, "frozen", False):  # PyInstaller 실행일 때만
        sys.stdout = open("stdout.log", "w", encoding="utf-8", buffering=1)
        sys.stderr = open("stderr.log", "w", encoding="utf-8", buffering=1)
    ```
    

이렇게 빌드하면 EXE 실행 시 **검은 cmd 창이 뜨지 않습니다.**

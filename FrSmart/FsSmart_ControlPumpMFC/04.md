...
```mermaid
flowchart TD
    A[Program Start] --> B[InitCVIRTE <br/>(LabWindows/CVI runtime init)]
    B --> C[GetProjectDir & Load UI panels<br/>(control_handle, acq_handle)]
    C --> D[InitProcess()]
    D --> D1[ReadConfigCom]
    D1 --> D2[ActivateCom (Open serial port)]
    D2 --> D3[Delay 1.5s - Wait for device ready]
    D3 --> E[InitProcess_MFC()<br/>(Read firmware version, board count via MODBUS)]
    E -->|Fail| Z[Exit]
    E -->|Success| F[InitConfig_MFC()<br/>(Load UI line index, calibration files)]
    F --> G[Start UI Timer]
    G --> H[RunUserInterface Loop]
    H --> I[User actions<br/>(Buttons, sliders)]
    H --> J[Periodic status check<br/>(Timer event)]
    I --> K[SetDutyBoard]
    J --> L[ReadRegister_MFC]
    K --> M[WriteSingleReg]
    L --> N[DecodeData_MFC]
    M --> O[MODBUS Write]
    L --> P[MODBUS Read]
    P --> Q[Device response received]
    Q --> R[DecodePacket, Error_MFC, UI update]
```
